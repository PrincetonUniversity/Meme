...
%If a network node wishes to decode whether attribute $A$ is
%associated with a packet, it must determine whether the packet
%belongs to any FEC which is associated with $A$. If $A$ is associated
%with $N$ FECs, each of which has its own index, the switch requires
%$N$ rules for decoding $A$. 
