...
Consider a middlebox traffic-steering scenario with $N$ different sequences of middleboxes.  If $\frac{N}{2}$ paths start with nodes ABC and the other $\frac{N}{2}$ start with ABD, then switch $B$ must use $N$ rules to determine whether to forward packets to $C$ or $D$.
