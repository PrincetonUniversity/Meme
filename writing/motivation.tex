...
The initial SDX system used flat tags to encode these distinct
...
% More generally, each AS shares its list of routes with a central
% controller, and every unique set of routes is assigned a flat tag. The
% controller then attaches these tags to every packet by announcing them
% to all peers as destination MAC addresses via ARP replies. The
% fine-grained forwarding policies are then modified by the controller to
% read tags before making a forwarding decision.  
